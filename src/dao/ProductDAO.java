...
public class ProductDAO {
	...
	public List<Product> findByPage(int pageNo,int pageSize) throws Exception{
		String sql="select * from `products` limit "+(pageNo-1)*pageSize+","+pageSize;
		List<Product> list=(List<Product>)qr.query(sql, new BeanListHandler(Product.class));
		return list;
	}
	//按id查询
	...
	public List<Product> findlike(String name) throws Exception{
		...
		List<Product> listp=(List<Product>)qr.query(sql, new BeanListHandler(Product.class));
		return listp;
	}
	//购买后库存减少
