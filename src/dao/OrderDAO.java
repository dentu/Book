...
public class OrderDAO {
	...
	public void copyCart(int userid,String orderid) throws Exception{
		String sql="INSERT INTO orderitem SELECT '"+orderid+"',productid,buynum FROM cart where userid=?";
	...
	//按订单编号查询订单明细
		public List<OrderItem> findItem(String oid) throws Exception{
			...
			List<OrderItem> list=(List<OrderItem>)qr.query(sql, new BeanListHandler(OrderItem.class),oid);
			return list;
		}
	//按用户查询订单(含订单明细的组合查询)
		public List<Order> findOrder(int uid) throws Exception{
			String sql="select * from orders where user_id=? order by ordertime desc";
			List<Order> list=(List<Order>)qr.query(sql, new BeanListHandler(Order.class),uid);
			for(Order o:list){
